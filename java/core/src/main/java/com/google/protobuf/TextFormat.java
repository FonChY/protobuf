...
public final class TextFormat {
  ...
  private static final class Tokenizer {
    ...
    private int lineColTrackingPos = 0;
    ...
    void nextToken() {
      ...
      while (lineColTrackingPos < pos) {
        if (text.charAt(lineColTrackingPos) == '\n') {
          ++line;
          column = 0;
        } else {
          ++column;
    ...
    private String nextTokenInternal() {
      ...
      if (isAlphaUnder(startChar)) { // Identifier
        ...
        while (++endPos != textLength) {
          ...
          if (c == startChar) {
            ++endPos;
            break; // Unescaped quote terminates the string
          } else if (c == '\n') {
            break; // Newline terminates (error during parsing)
          } else if (c == '\\') {
            if (++endPos == textLength) {
              break; // Escape into end-of-text terminates (error during parsing)
            } else if (this.text.charAt(endPos) == '\n') {
              break; // Escape into newline terminates (error during parsing)
            } else {
